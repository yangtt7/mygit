...
(function (factory) {
    ...
    $.fn.dateRangePicker = function (opt) {
        ...
        opt = $.extend(true,
            {
                autoClose: false,
                format: 'YYYY-MM-DD',			//Moment的日期格式
                separator: ' to ',				//分隔符合
                language: 'auto',				//预定义的语言是"en"和"cn"。你可以使用这个参数自定义语言。也可以设置为"auto"来让浏览器自己检测语言。
                startOfWeek: 'sunday',			// or monday  一周计算的开始时间
                getValue: function ()			//当从DOM元素中获取日期范围时会调用该函数，函数的上下文被设置为datepicker DOM。
                {
                    return $(this).val();
                },
                setValue: function (s)			//当向DOM元素中写入日期范围时调用该函数。
                {
                    if (!$(this).attr('readonly') && !$(this).is(':disabled') && s != $(this).val()) {
                        $(this).val(s);
                    }
                },
                startDate: false,				//定义用户允许的最早日期，格式和format相同。
                endDate: false,					//定义用户允许的最后日期，格式和format相同。
                time: {							//如果允许该参数就会添加时间的范围选择
                    enabled: false
                },
                minDays: 0,						//该参数定义日期范围的最小天数，如果设置为0，表示不限制最小天数。
                maxDays: 0,						//该参数定义日期范围的最大天数，如果设置为0，表示不限制最大天数。
                showShortcuts: false,			//显示或隐藏shortcuts区域
                shortcuts:						//定义快捷键按钮。
                {
                    //'prev-days': [1,3,5,7],
                    // 'next-days': [3,5,7],
                ...
                customShortcuts: [],			//定义自定义快捷键按钮。
                inline: false,					//使用inline模式渲染该日期选择器，而不是overlay模式。如果设置为t
                container: 'body',				//要进行渲染的日期选择器DOM元素
                alwaysOpen: false,				//如果使用inline模式，你可能希望在页面加载时就渲染日期选择器。该参数设置为true时会隐藏"close"按钮。
                singleDate: false,				//设置为true可以选择单个的日期。
                lookBehind: false,
                batchMode: false,				//自动批处理模式。(false / 'week' / 'month')
                duration: 200,
                stickyMonths: false,
                dayDivAttrs: [],
                dayTdAttrs: [],
